	...
	public List<UserBookDetails> getBooksTobeIssued() {
		...
		try {
			...
			System.out.println("getting the Details for User...");
			 Query q = session.createSQLQuery("select BUID,BID,UID, Date_Of_Issue ,Date_Of_Return ,  Approved ,username ,roll ,Name ,Author ,Publisher from record r left join user u on u.id = r.uid left join book b on r.bid = b.bookid where Date_Of_Issue is null");
			 	 r = q.getResultList();
	      	   	session.getTransaction().commit();
		}
		catch(Exception ex) {
